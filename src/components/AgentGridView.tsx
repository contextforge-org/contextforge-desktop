    ...
    <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {agents.map((agent) => (
        ...
        >
          ...
          <p className={`text-xs mb-2 truncate ${theme === 'dark' ? 'text-zinc-500' : 'text-gray-500'}`} title={agent.endpointUrl}>
            {agent.endpointUrl}
          </p>
          
          <div className="mb-2">
            <span className={`inline-block px-2 py-1 rounded text-xs ${theme === 'dark' ? 'bg-zinc-800 text-zinc-300' : 'bg-gray-100 text-gray-700'}`}>
          ...
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${agent.enabled ? 'bg-cyan-500' : theme === 'dark' ? 'bg-zinc-600' : 'bg-gray-400'}`} />
